        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <el-upload
                                ...
                                :action="getActionUrl"
                                :on-success="zhongzhihuUploadSuccess"
                                :on-error="zhongzhihuUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zhongzhihu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zhongzhihu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zhongzhihuIdNumber"
                                   header-align="center"
                                   label="种植户身份证号">
                        <template slot-scope="scope">
                            {{scope.row.zhongzhihuIdNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zhongzhihuEmail"
                                   header-align="center"
                                   label="邮箱">
                        <template slot-scope="scope">
                            {{scope.row.zhongzhihuEmail}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('zhongzhihu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zhongzhihu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('zhongzhihu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('zhongzhihu','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                password : null,
                zhongzhihuName : null,
                zhongzhihuPhoto : null,
                zhongzhihuPhone : null,
                zhongzhihuIdNumber : null,
                zhongzhihuEmail : null,
                newMoney : null,
                sexTypes : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zhongzhihu',//当前表表名,
                        sumColum : 'zhongzhihu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zhongzhihu_name',//分组字符串字段
                        // types : 'zhongzhihu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['zhongzhihuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "zhongzhihu/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            zhongzhihuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zhongzhihu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        ...
                        data: {
                            id:id,
//                            zhongzhihuTypes:zhongzhihuTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
